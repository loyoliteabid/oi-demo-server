...
export function mapToAirQuality(
  ...
  const dateTime = moment(
    ...
    ? moment(`${rawDate} ${rawTime}`, "DD/MM/YYYY HH.mm.ss").toDate() // Valid date
  ...
  const parseNumber = (value: string | undefined): number | null => {
    ...
    return !parsed || isNaN(parsed) ? null : parsed; // Return null if the parsed value is NaN
  };

  return {
    date: rawDate, // Keep the original date
    time: rawTime, // Keep the original time
    dateTime, // Store the combined valid Date object or null
...
export async function ingestFromCSV(filePath: string) {
  ...
  for await (const row of parsed.data) {
    try {
